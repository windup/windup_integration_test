...
def load_appliance_collections():
    ...
    return {ep.name: ep.resolve() for ep in iter_entry_points("rhamt.application_collections")}
...
class EntityCollections(object):
    ...
    _collection_cache = attr.ib(
        repr=False, eq=False, hash=False, init=False, default=attr.Factory(dict)
    )
